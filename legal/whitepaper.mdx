...
The platform supports a curated selection of BEP-20 assets, including INKY, USDT, BTCB, ETH and other major tokens. INKY and USDT have dedicated ecosystem roles (staking, Arena, fee payment, fiat on-ramp). All other assets support Buy/Sell via USDT and user holdings. See [Wallet](/features/wallet) for the full asset list and per-token features.
...
**Supported pairs:** All supported assets trade against USDT. Both Buy (USDT to asset) and Sell (asset to USDT) are available. See [Swap](/features/swap) for the full asset list.
