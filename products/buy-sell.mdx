---
title: Buy and Sell
description: How the Inkryptus buy/sell system works — USDT-centric routing, supported assets, quoting, and fees.
---

Inkryptus replaces the traditional crypto swap with a simplified buy and sell interface. Every trade uses **USDT as the central pair**. There are no direct routes between non-USDT assets (e.g., INKY→BTCB does not exist). To move between two assets, the user sells one for USDT, then buys the other with USDT.
...
<!-- IMAGE SUGGESTION: Diagram with USDT at the center and 8 assets around it, bidirectional arrows between each asset and USDT. No arrows between assets. -->

## Supported assets

All 8 assets trade against USDT. The USDT itself is the central pair and does not appear as a tradeable asset in the Buy/Sell screen.
...
A fixed fee of **US$3** applies to every buy or sell operation, regardless of the trade amount.
...
Buy/Sell handles only the internal asset conversion. For depositing USDT into your wallet (via external wallet, exchange, or fiat provider), see [Wallet](/products/wallet). For sending assets to an external address, see [Wallet — Withdrawals](/products/wallet#withdrawals).
...
<!-- IMAGE SUGGESTION: Diagram showing Buy/Sell scope: USDT in center, 8 assets around it with bidirectional arrows. Deposit and Withdrawal arrows pointing into/out of the USDT node, labeled "See Wallet". -->
